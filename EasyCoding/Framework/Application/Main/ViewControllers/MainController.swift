...
class MainController: UITabBarController, UITabBarControllerDelegate, ECViewControllerType, ECApiRequestable {
    ...
    override func viewDidLoad() {
        ...
        let c2 = ExampleController()
        let c3 = ExampleController()
        ...
        self.load(n1, segue: ECPresentSegue.TabBar(iconNamed: "未选图片", selectedNamed: "选中图片"))
        self.load(n2, segue: ECPresentSegue.TabBar(iconNamed: "未选图片", selectedNamed: "选中图片"))
        self.load(n3, segue: ECPresentSegue.TabBar(iconNamed: "未选图片", selectedNamed: "选中图片"))
        
        self.tabBar.items![0].title = "第一个"
        self.tabBar.items![1].title = "第二个"
        self.tabBar.items![2].title = "第三个"
        
        for item in self.tabBar.items! {
            item.imageInsets = .init(top: -2, left: 0, bottom: 2, right: 0)
            item.titlePositionAdjustment = .init(horizontal: 0, vertical: -5)
            item.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Style.Color.main], for: .selected)
            item.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Style.Color.Font.light], for: .normal)
        }
