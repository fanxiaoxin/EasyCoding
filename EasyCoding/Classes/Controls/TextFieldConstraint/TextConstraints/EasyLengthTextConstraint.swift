...
import UIKit
///长度限制
open class EasyLengthTextConstraint: EasyTextConstraint {
    ...
    open override func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if self.isCountByByte {
            if result.easy.lengthOfBytes > maxLength {
                return false
            }
        }else{
...
extension EasyTextConstraint {
    public static var phone: EasyTextConstraint {
        let c = EasyLengthTextConstraint(11)
        c.keyboardType = .phonePad
        return c
    }
    public static func max(length:Int,isCountByByte:Bool = false) -> EasyTextConstraint {
        let c = EasyLengthTextConstraint(length)
