...
open class EasyDateTextConstraint: EasyTextConstraint {
    ///可对该Picker设置属性
    ...
    func updateText() {
        self.textField?.text = _date?.easy.string(self.stringFormat)
    }
    func valueChanged() {
        self.updateText()
...
extension EasyCoding where Base: UITextField {
    public  var date: Date? {
        get{
            if let tc = self.textConstraint as? EasyDateTextConstraint {
                return tc.date
            }
            if let text = self.base.text {
                return Date(stringLiteral: text)
            }else{
                return nil
            }
        }
        set{
            if let tc = self.textConstraint as? EasyDateTextConstraint {
                ...
                self.base.text = newValue?.easy.string()
            }
        }
    }
}
...
extension EasyTextConstraint {
    public static func date(mode:UIDatePicker.Mode = .date, format: String, min: Date? = nil, max: Date? = nil) -> EasyTextConstraint {
        let tc = EasyDateTextConstraint()
