...
open class EasyNumberTextConstraint<T:Numeric>: EasyTextConstraint where T:LosslessStringConvertible,T:Comparable {
    ///超过最大值事件
    public let whenOutOfMax = EasyEventManager<T>()
    ...
    ///用于判断是否正在编辑
    private var _isECEditing: Bool = false
    ...
    public override var textField: UITextField?{
        didSet{
            if let tf = self.textField {
                ...
                _isECEditing = textField?.isFirstResponder ?? false
                _editingValue = nil
    ...
    func updateRawValue() {
        if _isECEditing {
            if let v = _editingValue {
                _rawValue = v
                _editingValue = nil
            }else if let text = self.textField?.text, !text.easy.isBlank {
                _rawValue = T(text)
            }else{
                _rawValue = nil
            }
...
extension EasyCoding where Base: UITextField {
    public func number<T:Numeric>() -> T? where T: LosslessStringConvertible,T:Comparable{
        if let tc = self.textConstraint as? EasyNumberTextConstraint<T> {
            return tc.value
        }
...
extension EasyTextConstraint {
    public static var money: EasyTextConstraint {
        return .double(format: "###,##0.00", decimalLength: 2, min: nil, max: nil)
    }
    public static func money(decimalLength:Int = 2, min:Double? = nil, max: Double? = nil) -> EasyTextConstraint {
    ...
    public static func double(format:String? = nil, decimalLength:Int = 0, min:Double? = nil, max: Double? = nil) -> EasyTextConstraint {
        let c = EasyNumberTextConstraint<Double>()
    ...
    public static func int(format:String? = nil, min:Int? = nil, max: Int? = nil) -> EasyTextConstraint {
        let c = EasyNumberTextConstraint<Int>()
        c.formatter.positiveFormat = format
        c.decimalLength = 0
        c.min = min
        c.max = max
