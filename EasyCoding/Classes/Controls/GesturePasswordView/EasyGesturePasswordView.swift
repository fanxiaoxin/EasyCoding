...
///手势解锁View
@IBDesignable open class EasyGesturePasswordView: UIControl,CAAnimationDelegate {
    ///最终值
    open var value : [Int] = []
    ///检查密码序列是否正确
...
extension EasyStyleSetting where TargetType: EasyGesturePasswordView {
    public static func unselected(image:UIImage) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.unselectedImage = image
        })
    }
    public static func selected(image:UIImage) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.selectedImage = image
        })
    }
    public static func unselected(_ color:UIColor) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.unselectedColor = color
        })
    }
    public static func selected(_ color:UIColor) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.color = color
        })
    }
    public static func error(_ color:UIColor) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    public static func padding(_ padding:CGFloat) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
