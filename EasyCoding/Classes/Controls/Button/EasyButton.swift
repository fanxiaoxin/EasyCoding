...
open class EasyButton: UIButton {
    ///图片位置
    open var imagePosition: EasyDirection = .left
    ///下划线宽度
...
extension EasyBuildable where Self: UIView {
    public static func easyButton(_ styles: EasyStyleSetting<EasyButton>...) -> EasyButton {
        return EasyButton().easy(styles: styles)
    }
}

extension EasyStyleSetting where TargetType: EasyButton {
    ///下划线
    public static func underline(_ color:UIColor? = nil, width: CGFloat = 1) -> EasyStyleSetting<TargetType> {
    ...
    ///下划线
    public static func underline(rgb color:UInt32, width: CGFloat = 1) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.underlineColor = UIColor.easy.rgb(color)
            target.underlineWidth = width
        })
    }
    ///图片位置
    public static func image(position:EasyDirection) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.imagePosition = position
        })
    }
    ///事件触发范围
    public static func hit(inset:UIEdgeInsets) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
