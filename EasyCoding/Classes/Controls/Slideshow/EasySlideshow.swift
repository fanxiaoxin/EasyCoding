...
open class EasySlideshow: UIView,UIScrollViewDelegate {
    ///点击事件
    let whenClick = EasyEventManager<CustomStringConvertible>()
//    public let onClick = EasyEvent
    ...
    public func reloadData(){
        self.containerView.easy.removeAllSubviews()
...
//                    }
                ...
                if self.containerView.subviews.count > 0 {
                    self.containerView.subviews[0].easy.layout(self.containerView, .left, .top, .bottom)
                    if self.containerView.subviews.count > 1 {
                         for i in 1..<self.containerView.subviews.count {
                            self.containerView.subviews[i - 1]
                                .easy.layout(self.containerView.subviews[i],
                                           .rightLeft)
                             self.containerView.subviews[i].easy.layout(self.containerView, .top, .bottom)
                        }
                    }
                    self.containerView.subviews.last!.easy.layout(self.containerView, .right)
                }
//                self.scrollView.layoutIfNeeded()
    ...
    @IBInspectable public let pageControl : EasyPageControl
    ...
    override public init(frame: CGRect) {
        ...
        self.pageControl = EasyPageControl()
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if self.autoTransform {
            ...
            for i in 0..<self.containerView.subviews.count {
                ...
                let rate = min(abs(view.bounds.origin.x - offset) / scrollView.bounds.size.width, 1)
                let scale = 1 * (0.85 + 0.15 * (1 - rate))
                //            var transform = CGAffineTransform(translationX: (offset - view.left) * 0.3 , y: 0)
                //            transform = transform.scaledBy(x: scale, y: scale)
                var transform = CGAffineTransform(scaleX: scale, y: scale)
                let sb:CGFloat = offset > view.bounds.origin.x ? 1 : -1
                let translate = min(abs(offset - view.bounds.origin.x),scrollView.bounds.size.width) / scrollView.bounds.size.width
...
extension EasyBuildable where Self: UIView {
    public static func easySlideshow(_ styles: EasyStyleSetting<EasySlideshow>...) -> EasySlideshow {
        return EasySlideshow().easy(styles: styles)
    }
}
extension EasyStyleSetting where TargetType: EasySlideshow {
    public static func page(color:UIColor?) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pageIndicatorTintColor = color
        })
    }
    public static func page(selected color:UIColor?) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.currentPageIndicatorTintColor = color
        })
    }
    public static func page(size:CGSize) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pointSize = size
        })
    }
    public static func page(spacing:CGFloat) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pointSpacing = spacing
        })
    }
    public static func page(offset:CGFloat) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    public static func page(size:CGSize, spacing: CGFloat, color normal: UIColor?, selected current: UIColor?) -> EasyStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pointSize = size
            target.pageControl.pointSpacing = spacing
