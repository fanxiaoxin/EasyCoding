...
public protocol EasyPageEventDelegate:AnyObject {
    func performEvent(_ name:String, params: Any?)
    ///未被触发的事件会调用该方法转发出去
    func forwardEvent(_ name:String, params: Any?)
}
extension EasyPageEventDelegate where Self: NSObjectProtocol {
...
///页面
open class EasyPage: UIView {
    let eventKey = "EasyPageEvent"
    class Event {
        var name: String
        var params: Any?
    ...
    //****** 事件绑定 ******/
    open weak var eventDelegate: EasyPageEventDelegate?
    ///绑定事件
    ...
    open func bind(_ control:UIControl,event name:String, for event:UIControl.Event, params: Any? = nil) {
        control.easy.setAssociated(object: Event(name, params), key: eventKey)
    ...
    open func bind(_ gr:UIGestureRecognizer,event name:String, params: Any? = nil) {
        gr.easy.setAssociated(object: Event(name, params), key: eventKey)
        gr.addTarget(self, action: #selector(self.triger(gestureRecognizer:)))
    }
    ///重新绑定事件参数
    open func rebind(_ control:NSObject, params: Any?) {
        if let event: Event = control.easy.getAssociated(object: eventKey) {
            event.params = params
        }
    }
    ...
    @objc open func triger(gestureRecognizer:UIGestureRecognizer) {
        if let event: Event = gestureRecognizer.easy.getAssociated(object: eventKey) {
            self.triget(event: event.name, params: event.params)
        }
    }
    ...
    open func fillStraightBangs() -> UIView? {
        if UIDevice.current.easy.isStraightBangs {
            return self.easy.add(.view(.bg(.white), .height(33)), layout: .top, .marginX).base
        }else{
            return nil
        }
    }
...
extension EasyPage {
    public func button(_ button: UIButton, event:String, params: Any? = nil ,_ style: EasyStyleSetting<UIButton>...) -> UIButton {
        self.bind(button, event: event, params: params)
        style.forEach({$0.action(button)})
        return button
    }
    public func button(_ event:String, params: Any? = nil ,_ style: EasyStyleSetting<UIButton>...) -> UIButton {
        let button = UIButton()
        self.bind(button, event: event, params: params)
        style.forEach({$0.action(button)})
        return button
    }
    public func easyButton(_ event:String, params: Any? = nil ,_ style: EasyStyleSetting<EasyButton>...) -> EasyButton {
