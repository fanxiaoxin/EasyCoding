...
///键盘适配的插件
open class EasyViewControllerKeyboardAdapter: NSObject, YYKeyboardObserver {
    ///设置时如果keyboardConstraint未设置则会自动去搜索最外层的ScrollView，若没有则最下层的View并设置
    open weak var controller: UIViewController? {
        willSet {
            if let old = self.controller {
                old.easy.event.unregister(event: .didAppear(.before), target: self)
                old.easy.event.unregister(event: .willDisappear(.before), target: self)
            }
        }
        didSet {
            if let controller = self.controller {
                controller.easy.event.register(event: .didAppear(.before), target: self, action: #selector(self.beforeViewDidAppear(_:)))
                controller.easy.event.register(event: .willDisappear(.before), target: self, action: #selector(self.beforeViewWillDisappear(_:)))
            }
    ...
    @objc open func beforeViewDidAppear(_ animated: Bool) {
        self.tryAutoSetKeyboardConstraint()
        //添加键盘管理
    ...
    @objc open func beforeViewWillDisappear(_ animated: Bool) {
        //去掉键盘管理
        if self.keyboardConstraint != nil {
