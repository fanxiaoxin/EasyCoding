...
///授权委托
public protocol ECPreconditionDelegate: AnyObject {
    ///授权完成
    func ECPrecondition(_ precondition:Any, passed:Bool)
}
///代表一个权限
open class ECPrecondition<InputType,OutputType>  {
    ///鉴权成功操作
    ...
    ///上一个前置条件
    public private(set) var previous:ECPrecondition<InputType,OutputType>?
    
    ///委托
    public weak var delegate: ECPreconditionDelegate?
    public init() {
        self.previous = self.previousPrecondition()
        self.previous?.successHandler = { [weak self] in self?.verify() }
    }
    open func previousPrecondition() -> ECPrecondition<InputType,OutputType>? {
        return nil
    }
    ...
    open func finished(_ passed:Bool) {
        ...
        self.delegate?.ECPrecondition(self, passed: passed)
    }
}
