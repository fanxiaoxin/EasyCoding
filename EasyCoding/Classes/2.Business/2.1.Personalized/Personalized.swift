...
///个性化相关资源提供者
public protocol ECPersonalizedProviderType {
    ///应用到某个操作
    func apply(to target: ECPersonalizable)
}
...
extension ECPersonalizedManagerType {
    ...
    public func register(_ target: ECPersonalizable) {
    ...
    public func apply() {
        if let provider = self.provider {
            self.targets.forEach { (target: ECPersonalizable) in
                provider.apply(to: target)
            }
        }
    }
