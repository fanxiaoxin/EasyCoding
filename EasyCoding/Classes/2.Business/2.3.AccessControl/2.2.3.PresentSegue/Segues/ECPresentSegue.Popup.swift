...
extension ECPresentSegue {
    ...
    open class Popup: Window {
        ...
        open override func animation(for view: UIView) -> ECPresentAnimationType? {
        ...
        ///默认动画
        private func defaultAnimation(for view: UIView) -> ECPresentAnimationType {
            ...
            if top != bottom && left == right {
                if top {
                    return ECPresentAnimation.SlideOut(direction: .top)
                }else{
                    return ECPresentAnimation.SlideOut(direction: .bottom)
                }
            } else if left != right && top == bottom {
                if left {
                    return ECPresentAnimation.SlideOut(direction: .left)
                }else{
                    return ECPresentAnimation.SlideOut(direction: .right)
                }
            }else{
                return ECPresentAnimation.Popup()
            }
        }
    }
    ///Popup的常规场景
    ...
    ///Popup的常规场景
    public static func popup(alpha: CGFloat? = nil, animation: ECPresentAnimationType? = nil) -> Popup {
