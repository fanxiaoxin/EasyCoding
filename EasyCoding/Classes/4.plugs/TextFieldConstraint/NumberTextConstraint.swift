...
open class ECNumberTextConstraint<T:Numeric>: ECTextConstraint where T:LosslessStringConvertible,T:Comparable {
    ///超过最大值事件
    public let whenOutOfMax = ECEventManager<T>()
...
extension EC.NamespaceImplement where Base: UITextField {
    public func number<T:Numeric>() -> T? where T: LosslessStringConvertible,T:Comparable{
        if let tc = self.textConstraint as? ECNumberTextConstraint<T> {
            return tc.value
        }
...
extension ECTextConstraint {
    public static var money: ECTextConstraint {
        return .double(format: "###,##0.00", decimalLength: 2, min: nil, max: nil)
    }
    public static func money(decimalLength:Int = 2, min:Double? = nil, max: Double? = nil) -> ECTextConstraint {
    ...
    public static func double(format:String? = nil, decimalLength:Int = 0, min:Double? = nil, max: Double? = nil) -> ECTextConstraint {
        let c = ECNumberTextConstraint<Double>()
    ...
    public static func int(format:String? = nil, min:Int? = nil, max: Int? = nil) -> ECTextConstraint {
        let c = ECNumberTextConstraint<Int>()
        c.formatter.positiveFormat = format
        c.decimalLength = 0
        c.min = min
        c.max = max
