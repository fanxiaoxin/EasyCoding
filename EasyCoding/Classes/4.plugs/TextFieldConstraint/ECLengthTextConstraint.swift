...
import UIKit
///长度限制
open class ECLengthTextConstraint: ECTextConstraint {
...
extension ECTextConstraint {
    public static var phone: ECTextConstraint {
        let c = ECLengthTextConstraint(11)
        c.keyboardType = .phonePad
        return c
    }
    public static func max(length:Int,isCountByByte:Bool = false) -> ECTextConstraint {
        let c = ECLengthTextConstraint(length)
