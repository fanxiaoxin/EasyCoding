...
open class ECTextConstraint: NSObject,UITextFieldDelegate {
    public var otherConstraint: ECTextConstraint?
...
extension EC.NamespaceImplement where Base: UITextField {
    public var textConstraint : ECTextConstraint? {
        return objc_getAssociatedObject(self.base, &fx_key_textfield_textconstraiint) as? ECTextConstraint
    }
    public func set(textConstraint: ECTextConstraint?) {
        ///调用静态属性执行Swizzle，因为静态属性只执行1次
        let _ = ECTextConstraint.swizzleTextFieldDelegateMethod
...
extension ECStyleSetting where TargetType: UITextField {
    public static func constraint(_ constraint:ECTextConstraint) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.easy.set(textConstraint:constraint)
        })
    }
    public static func constraints(_ constraints:ECTextConstraint...) -> ECStyleSetting<TargetType> {
