...
open class ECDateTextConstraint: ECTextConstraint {
    ///可对该Picker设置属性
...
extension EC.NamespaceImplement where Base: UITextField {
    public  var date: Date? {
        get{
            if let tc = self.textConstraint as? ECDateTextConstraint {
                return tc.date
            }
...
extension ECTextConstraint {
    public static func date(mode:UIDatePicker.Mode = .date, format: String, min: Date? = nil, max: Date? = nil) -> ECTextConstraint {
        let tc = ECDateTextConstraint()
