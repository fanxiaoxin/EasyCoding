//
//  ECCollectionViewDataSource.swift
//  EasyCoding
//
...
open class ECCollectionViewCell<ModelType>: UICollectionViewCell {
...
open class ECCollectionSupplementaryView<SectionType>: UICollectionReusableView {
    public override init(frame: CGRect) {
        super.init(frame: frame)
        self.load()
    }
...
open class ECCollectionViewDataSource<DataProviderType: ECDataListProviderType>: ECListControlDataSource<DataProviderType>, UICollectionViewDataSource {
    ///当前操作的tableView
    ...
    open func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if let model = self.sections?[indexPath.section] {
            if let identifier = self.identifierForHeader(model, indexPath) {
                let view = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: identifier, for: indexPath) as! ECCollectionSupplementaryView<SectionType>
                view.load(model: model, indexPath: indexPath)
                return view
            }else if let identifier = self.identifierForFooter(model, indexPath) {
                let view = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: identifier, for: indexPath) as! ECCollectionSupplementaryView<SectionType>
                view.load(model: model, indexPath: indexPath)
                return view
            }
        }
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let model = self.datas?[indexPath.section][indexPath.row] {
            ...
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as! ECCollectionViewCell<ModelType>
