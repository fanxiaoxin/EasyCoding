...
public enum ECParagraphStyleSetter {
...
extension EC.NamespaceImplement where Base: NSMutableParagraphStyle {
    public func apply(setter: ECParagraphStyleSetter) {
        switch setter {
...
extension ECStringAttribute {
    ///段落样式
    public static func paragraph(_ setters: ECParagraphStyleSetter...) -> ECStringAttribute {
        let value = NSMutableParagraphStyle()
        setters.forEach({ value.easy.apply(setter: $0) })
