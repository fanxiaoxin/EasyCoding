...
public struct ECStringAttribute {
    ...
    ///指定作用范围, len<=0则代表到结尾
    public static func range(_ loc: Int,_ len: Int,_ attrs: ECStringAttribute...) -> ECStringAttribute {
    ...
    ///指定作用范围, len<=0则代表到结尾
    public static func range(_ range: NSRange,_ attrs: ECStringAttribute...) -> ECStringAttribute {
        var values: [NSAttributedString.Key: Any] = [:]
        for a in attrs {
            values.merge(a.attributes) { (v, _) -> Any in return v }
        }
...
extension EC.NamespaceImplement where Base == String {
    public func attr(_ attrs: ECStringAttribute...) -> NSAttributedString {
        if attrs.count == 0 {
    ...
    public func mutableAttr(_ attrs: ECStringAttribute...) -> NSMutableAttributedString {
        let str = NSMutableAttributedString(string: self.base)
        attrs.forEach({ $0.apply(str) })
        return str
    }
}
extension EC.NamespaceImplement where Base: NSAttributedString {
    public func attr(_ attrs: ECStringAttribute...) -> NSMutableAttributedString {
        let str = NSMutableAttributedString(attributedString: self.base)
        attrs.forEach({ $0.apply(str) })
        return str
    }
}
extension EC.NamespaceImplement where Base: NSMutableAttributedString {
    @discardableResult
    public func attr(_ attrs: ECStringAttribute...) -> Base {
        attrs.forEach({ $0.apply(self.base) })
        return self.base
    }
    @discardableResult
    public func reg(_ reg: String, options:  NSRegularExpression.Options = .caseInsensitive, _ attrs: ECStringAttribute...) -> Base {
        var values: [NSAttributedString.Key: Any] = [:]
        for a in attrs {
            values.merge(a.attributes) { (v, _) -> Any in return v }
        }
        ...
        reg?.enumerateMatches(in: self.base.string, options: .reportProgress, range: NSRange(location: 0, length: self.base.string.count)) { (result, flags, _) in
            if let r = result {
                ECStringAttribute(values, range: r.range).apply(self.base)
            }
        }
        return self.base
    }
