...
extension ECStyleSetting where TargetType: UITableView {
    ///行高
    public static func row(height:CGFloat) -> ECStyleSetting<TargetType> {
    ...
    ///自动行高
    public static func row(estimated height:CGFloat) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.estimatedRowHeight = height
    ...
    public static func header(_ view: UIView) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.tableHeaderView = view
        })
    }
    public static func footer(_ view: UIView) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.tableFooterView = view
        })
    }
    ///将底部置为空View，可清除多余的分隔线
    public static var emptyFooter: ECStyleSetting<TargetType> {
        return .init(action: { (target) in
