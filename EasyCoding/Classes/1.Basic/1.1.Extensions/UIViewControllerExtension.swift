...
extension EC.NamespaceImplement where Base == UIViewController {
    public static var current: UIViewController? {
        if var target = UIApplication.shared.delegate?.window??.rootViewController {
            repeat {
                if let c = target.easy.__upLayerViewController() {
                    target = c
                }else{
                    break
                }
    ...
    ///获取上一层Controller
    private func __upLayerViewController() -> UIViewController? {
...
extension EC.NamespaceImplement where Base: UINavigationController {
