...
///日期单位
public enum ECDateUnit: Int, Comparable {
    ///年
    case year = 6
    ///月
    case month = 5
    ...
    ///毫秒
    case nanosecond = 0
    public static func < (lhs: ECDateUnit, rhs: ECDateUnit) -> Bool {
        return lhs.rawValue < rhs.rawValue
    }
}
///日期值
public enum ECDateValue: Equatable {
    ///年
    case year(Int)
    ///月
    case month(Int)
...
///常用格式化参数
public enum ECDateFormat: CustomStringConvertible {
...
extension ECDateFormat {
    ///yyyy-MM-dd
    public static var date: ECDateFormat { return .date("-")}
    ///HH:mm:ss
    public static var time: ECDateFormat { return .time(":")}
    ///yyyy-MM-dd HH:mm:ss
    public static var datetime: ECDateFormat { return .datetime("-", ":", " ")}
}

extension EC.NamespaceImplement where Base == Date {
    ///转成objc对象
    public var objc: NSDate {
        return self.base as NSDate
    }
    ///实例
    public static func instance(_ values: ECDateValue...) -> Date {
    ...
    public func format(_ format: ECDateFormat) -> String {
        return self.string(format.description)
    }
    ///两个日期相差
    public func since(_ date: Date, for unit: ECDateUnit) -> Int {
        switch unit {
...
extension EC.NamespaceImplement where Base == String {
    ...
    ///实例
    public func date(format: ECDateFormat) -> Date {
        return self.date(format.description)
    }
}
extension EC.NamespaceImplement where Base == TimeInterval {
    ///将秒显示为天时分秒的剩余时长描述，start至少为天, fixed代表固定会出现的单位，若高于fixed的单位值为0时不出现
    public func remainingDuration(_ endDesc:String = "已结束",start: ECDateUnit = .day, end: ECDateUnit = .minute, fixed: ECDateUnit = .hour) -> String {
