...
public extension ECBadge {
    ///设置视图，frame为相对于中心偏移的位置(默认为右上角, 如tabBar则为标准图标大小(25 x 25)的右上角)，圆角和大小需要自己设置
    static func view(_ view:UIView)-> Self {
        return .view(view, position: .rightTop(0, 0))
    }
    ///Label
    static func label(position: Position = .rightTop(0, 0), style: ECStyleSetting<ECLabel>...) -> Self {
        return .view(ECLabel.easy(styles: style), position: position)
    }
    ///数量
    static func value(_ value: Int, font size: CGFloat = 10, color: UIColor = .systemRed, position: Position = .rightTop(0, 5), style: ECStyleSetting<ECLabel>...) -> Self {
        let label = ECLabel.easy(.text(value.description), .font(size: size), .bg(color), .color(.white), .padding(.easy(x: 3, y: 1)))
...
extension EC.NamespaceImplement where Base: UIView {
    ///显示角标
    public func setBadge(_ badge: ECBadge) {
        badge.set(for: self.base)
    }
}

extension EC.NamespaceImplement where Base: UITabBar {
    ///显示角标
    public func setBadge(_ badge: ECBadge, at index: Int) {
