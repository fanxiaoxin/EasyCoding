...
open class ECButton: UIButton {
    ///图片位置
    open var imagePosition: ECDirection = .left
    ///下划线宽度
...
extension ECBuildable where Self: UIView {
    public static func easyButton(_ styles: ECStyleSetting<ECButton>...) -> ECButton {
        return ECButton().easy(styles: styles)
    }
}

extension ECStyleSetting where TargetType: ECButton {
    ///下划线
    public static func underline(_ color:UIColor? = nil, width: CGFloat = 1) -> ECStyleSetting<TargetType> {
    ...
    ///下划线
    public static func underline(rgb color:UInt32, width: CGFloat = 1) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
