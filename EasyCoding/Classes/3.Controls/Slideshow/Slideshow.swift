...
open class ECSlideshow: UIView,UIScrollViewDelegate {
    ///点击事件
    let whenClick = ECEventManager<CustomStringConvertible>()
//    public let onClick = ECEvent
...
//                self.scrollView.layoutIfNeeded()
    ...
    @IBInspectable public let pageControl : ECPageControl
    ...
    override public init(frame: CGRect) {
        ...
        self.pageControl = ECPageControl()
...
extension ECStyleSetting where TargetType: ECSlideshow {
    public static func page(color:UIColor?) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pageIndicatorTintColor = color
        })
    }
    public static func page(selected color:UIColor?) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.currentPageIndicatorTintColor = color
        })
    }
    public static func page(size:CGSize) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pointSize = size
        })
    }
    public static func page(spacing:CGFloat) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pointSpacing = spacing
        })
    }
    public static func page(offset:CGFloat) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    public static func page(size:CGSize, spacing: CGFloat, color normal: UIColor?, selected current: UIColor?) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.pageControl.pointSize = size
            target.pageControl.pointSpacing = spacing
