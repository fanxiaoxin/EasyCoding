...
open class ECPopupKeyboard<InputType>: UIView, ECKeyboardType, UIGestureRecognizerDelegate {
...
//        let startScale = CGPoint(x: max(min(1, rect.size.width / frame.size.width), 0.01), y: 0.01)
        ...
        self.contentView.easy.show(animation: ECPresentAnimation.Popup(anchor: anchor, startScale: startScale))
    }
    open func dismiss() {
