...
//                size.height -= self.padding.top + self.padding.bottom
                ...
                if let idx = at.easy.characterIndex(at: p, for: size, numberOfLines: self.numberOfLines) {
                    if let eventHandler = attrText.attribute(ECEventAttributedStringKey, at: idx, effectiveRange: nil) as? (()->Void) {
                        eventHandler()
                    }
                }
            }
...
extension ECStyleSetting where TargetType: ECLabel {
    public static func italic(_ italic:Bool = true) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.isItalic = italic
        })
    }
    public static func outline(_ size:CGFloat = 1) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.outlinedSize = size
        })
    }
    public static func outline(color:UIColor) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
...
public let ECEventAttributedStringKey = NSAttributedString.Key("ECEvent")

extension ECStringAttribute {
    ///链接，只有ECLabel能响应
    public static func event(_ handler: @escaping ()-> Void) -> ECStringAttribute {
        return .init(ECEventAttributedStringKey, handler)
    }
}
