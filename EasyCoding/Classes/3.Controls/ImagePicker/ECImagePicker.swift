...
extension EC.NamespaceImplement where Base: UIViewController {
    ...
    public func selectImage(allowEdit:Bool = true,finished:@escaping (UIImage)->Void) {
        ECImagePicker.selectImage(self.base, allowEdit: allowEdit, finished: finished)
    }
    ///直接调用相册
    public func takePicture(allowEdit:Bool = true,finished:@escaping (UIImage)->Void) {
        ECImagePicker.takePicture(self.base, allowEdit: allowEdit, finished: finished)
    }
    ///直接调用拍照
    public func takePhoto(allowEdit:Bool = true,finished:@escaping (UIImage)->Void) {
        ECImagePicker.takePhoto(self.base, allowEdit: allowEdit, finished: finished)
    }
    ///直接调用拍视频
    public func takeVideo(maximumDuration:TimeInterval,finished:@escaping (URL)->Void) {
        ECImagePicker.takeVideo(self.base, maximumDuration: maximumDuration, finished: finished)
    }
}
