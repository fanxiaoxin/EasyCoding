...
open class ECCurtainController: UIViewController {
...
//        }
        ...
        self.container.snp.updateConstraints { (make) in
            make.bottom.equalTo(self.view).offset(self.container.bounds.size.height)
        }
        to.view.layoutIfNeeded()
        self.container.snp.updateConstraints { (make) in
        ...
        UIView.animate(withDuration: 0.5, animations: { () -> Void in
            to.view.layoutIfNeeded()
                ...
                self.completion?()
                self.perform(#selector(self.hide), with: nil, afterDelay: 1.5)
        })
    }
    @objc open func hide(){
        self.container.snp.updateConstraints { (make) in
