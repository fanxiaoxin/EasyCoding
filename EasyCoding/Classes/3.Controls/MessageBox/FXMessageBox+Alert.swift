...
//        let view = self.wrapView(label)
    ...
    public class func input<TextField: UITextField>(title:String?, message:String?,placeholder:String?,action:@escaping ((String?)->Bool)) -> TextField {
        ...
        label.font = label.font?.easy.bold
        let input = TextField()
        input.placeholder = placeholder
        let contentView = UIView()
        contentView.addSubview(label)
        contentView.addSubview(input)
        contentView.easy.add(label, layout: .top(15), .marginX)
...
extension ECStyleSetting where TargetType: ECAlertController {
    ///标题样式
    public static func text(_ styles: ECStyleSetting<UILabel>...) -> ECStyleSetting<TargetType> {
