...
import UIKit
///弹窗
open class ECAlertView: ECPage {
    ///单独配置
    public var config: ECAlertConfig? = nil
    ///按钮类型
    ...
    public struct Button {
        ...
        public init(sizeRatio: CGFloat = 1, type: ECAlertView.ButtonType = .normal, text: String?, action: (() -> Void)?) {
    ...
    ///加载配置
    open func loadConfig<ReturnType>(_ action: (ECAlertConfig) -> ReturnType) -> ReturnType{
        if let cfg = self.config {
            if cfg.isBaseOnDefault {
                _ = action(ECAlertConfig.default)
            }
            return action(cfg)
        }else{
            return action(ECAlertConfig.default)
        }
    }
    ///加载样式
    public func apply<ViewType: UIView>(_ view: ViewType, _ action: (ECAlertConfig) -> ECControlConfig<ViewType>?) {
