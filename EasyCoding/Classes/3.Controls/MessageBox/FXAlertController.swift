...
open class ECAlertController: UIViewController,YYKeyboardObserver {
    ...
    override open func viewDidLoad() {
        ...
        if let contentView = self.contentView {
            ...
            if let buttons = self.buttons, buttons.count > 0 {
                //两个按钮时横向并列，其他纵向并列
                if buttons.count > 2 {
                    var btn = buttonsView.easy.add(self.createButton(0), layout: .top, .marginX)
                    btn.base.snp.makeConstraints { (make) in
                        make.height.equalTo(40).priority(.low)
                    }
                    for i in 1..<buttons.count {
                        btn = btn.next(self.createButton(i), layout: .bottomTop(CGFloat.easy.pixel), .paddingX, .height)
                    }
                    btn.parent(.bottom)
                }else{
                    var btn = buttonsView.easy.add(self.createButton(0), layout: .left, .marginY)
                    btn.base.snp.makeConstraints { (make) in
                        make.height.equalTo(40).priority(.low)
                    }
                    for i in 1..<buttons.count {
                        btn = btn.next(self.createButton(i), layout: .rightLeft(CGFloat.easy.pixel), .paddingY, .height, .width)
                    }
                    btn.parent(.right)
                }
            }else{
    ...
    override open func loadView() {
        ...
        separator.snp.makeConstraints({ (make) in
            ...
            make.left.equalTo(container).offset(self.stylesheet.padding)
            make.right.equalTo(container).offset(-self.stylesheet.padding)
            make.height.equalTo(CGFloat.easy.pixel)
        })
...
extension ECAlertController: ECStyleSetable { }
extension ECStyleSetting where TargetType: ECAlertController {
    ///遮罩层
    public static func cover(color:UIColor) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.backgroundView.backgroundColor = color
        })
    }
    ///遮罩层
    public static func cover(rgb color:UInt32) -> ECStyleSetting<TargetType> {
        return cover(color: UIColor.easy.rgb(color))
    }
    ///间距
    public static func padding(_ padding:CGFloat) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    ///标题样式
    public static func title(_ styles: ECStyleSetting<UILabel>...) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            styles.forEach({$0.action(target.titleLabel)})
        })
    }
    ///按钮边距
    public static func button(padding: UIEdgeInsets) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    ///按钮间距
    public static func button(spacing: CGFloat) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            let buttons = target.buttonsView.subviews
            //两个按钮时横向并列，其他纵向并列
            if buttons.count > 2 {
    ...
    ///所有按钮样式
    public static func button(_ styles: ECStyleSetting<UIButton>...) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    ///突出按钮样式
    public static func button(destructive styles: ECStyleSetting<UIButton>...) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    ///普通按钮样式
    public static func button(normal styles: ECStyleSetting<UIButton>...) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            for idx in 0..<target.buttonsView.subviews.count {
    ...
    ///分割线颜色
    public static func separator(color: UIColor) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
    ...
    ///分割线颜色
    public static func separator(rgb color: UInt32) -> ECStyleSetting<TargetType> {
        return separator(color: UIColor.easy.rgb(color))
    }
    ///分割线边距
    public static func separator(padding: CGFloat) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
