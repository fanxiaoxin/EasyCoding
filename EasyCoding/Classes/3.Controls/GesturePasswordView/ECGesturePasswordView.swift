...
///手势解锁View
@IBDesignable open class ECGesturePasswordView: UIControl,CAAnimationDelegate {
    ///最终值
    open var value : [Int] = []
    ///检查密码序列是否正确
...
extension ECStyleSetting where TargetType: ECGesturePasswordView {
    public static func unselected(image:UIImage) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.unselectedImage = image
        })
    }
    public static func selected(image:UIImage) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.selectedImage = image
        })
    }
    public static func unselected(_ color:UIColor) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.unselectedColor = color
        })
    }
    public static func selected(_ color:UIColor) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
            target.color = color
        })
    }
    public static func error(_ color:UIColor) -> ECStyleSetting<TargetType> {
        return .init(action: { (target) in
