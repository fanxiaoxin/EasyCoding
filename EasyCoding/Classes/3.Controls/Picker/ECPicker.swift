...
extension EC.NamespaceImplement where Base: UIView {
    @discardableResult
    public func pick(_ title:String?,contentView:UIView,completion:@escaping ()->Bool) -> ECPickerView{
        let pickerView = ECPickerView(title: title, contentView: contentView)
    ...
    public func pick(_ title:String?,date:Date?=nil,mode:UIDatePicker.Mode,min:Date?=nil,max:Date?=nil,completion: @escaping (Date?)->Bool){
        let pickerView = ECDatePickerView(title: title ?? "请选择日期", mode: mode)
        if let d  = date {
    ...
    public func pick(_ title:String?,options:[[String]],selected:[Int]?,attributes:[NSAttributedString.Key:Any]? = nil,completion:@escaping ([Int])->Bool) {
        let pickerView = ECOptionsPickerView(title: title ?? "请选择", options:options,attributes:attributes)
        pickerView.confirmAction = {
