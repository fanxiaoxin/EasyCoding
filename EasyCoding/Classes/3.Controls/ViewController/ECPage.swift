...
public protocol ECPageEventDelegate:AnyObject {
    func performEvent(_ name:String, params: Any?)
    ///未被触发的事件会调用该方法转发出去
    func forwardEvent(_ name:String, params: Any?)
}
extension ECPageEventDelegate where Self: NSObjectProtocol {
...
///页面
open class ECPage: UIView {
    let eventKey = "ECPageEvent"
    class Event {
        var name: String
        var params: Any?
    ...
    //****** 事件绑定 ******/
    open weak var eventDelegate: ECPageEventDelegate?
    ///绑定事件
...
extension ECPage {
    public func button(_ button: UIButton, event:String, params: Any? = nil ,_ style: ECStyleSetting<UIButton>...) -> UIButton {
        self.bind(button, event: event, params: params)
        style.forEach({$0.action(button)})
        return button
    }
    public func button(_ event:String, params: Any? = nil ,_ style: ECStyleSetting<UIButton>...) -> UIButton {
        let button = UIButton()
        self.bind(button, event: event, params: params)
        style.forEach({$0.action(button)})
        return button
